...
public class WebSocketManager : MonoBehaviour
{
    ...
    private void _HandleOnErrorWebSocket()
    {
        ...
        Logging.Log("OnError: Previous status=" + connectionStatus + ", IsAlive=" + (ws?.IsAlive ?? false));
    ...
    private void _HandleOnCloseWebSocket()
    {
        if (connectionStatus == ConnectionStatus.DISCONNECTED) return;
        connectionStatus = ConnectionStatus.DISCONNECTED;
        Logging.Log("OnClose: Previous status=" + connectionStatus + ", IsAlive=" + (ws?.IsAlive ?? false) + ", UserLogout=" + UserLogout);
        UnityMainThread.instance.AddJob(() =>
        {
            UIManager.instance.showLoginScreen(false);
            if (!UserLogout)
    ...
    private void _HandleOnOpenWebSocket()
    {
        ...
        UserLogout = true;
        Logging.Log("OnOpen: Status changed to " + connectionStatus + ", Pending jobs=" + jobsResend.Count);
        _OnConnectCb?.Invoke();
