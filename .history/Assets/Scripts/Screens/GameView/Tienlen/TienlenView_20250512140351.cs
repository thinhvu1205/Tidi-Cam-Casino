...
public class TienlenView : GameView
{
   ...
   [Seria]
    ...
    private void countDownTimeToStart(int time)
    {
        ...
        {
            timeToStart = time;
            timeToStartText.text = time.ToString();
            timeToStartText.gameObject.SetActive(true);
            timeToStartText.transform.localScale = Vector3.one;
            timeToStartText.transform.DOScale(Vector3.one * 1.5f, 0.5f).SetLoops(-1, LoopType.Yoyo);
        }
        else
        {
