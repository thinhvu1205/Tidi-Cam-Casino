...
public class TienlenView : GameView
{
    ...
    {
        // 1. Play sound và reset state
        SoundManager.Instance.PlaySound("sounds/start_game");
        beforeStartGame();

        // 2. Setup animation và UI
        m_AniStart.gameObject.SetActive(true);
        m_AniStart.AnimationState.SetAnimation(0, "animation", false);
        m_BgStart.SetActive(false);
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            // Set số lá bài hiển thị
            player.playerView.UpdateNumberCard(player == thisPlayer ? 13 : 0);
        }
        ...
        gameSequence.AppendCallback(() => {
            ...
            initPlayerCard();
            SetTurn(turnNameCurrent, timeTurn);

            // Show hint cho người đánh đầu
            ...
            for (int j = 0; j < listCard.Count; j++) 
            {
                ...
                else
                {
                    ...
                    cardSequence.AppendInterval(timedelay)
                        ...
                        .AppendCallback(() => {
                            // Update số lá bài
                            player.playerView.UpdateNumberCard(player.playerView.GetNumberCard() + 1);
                        });
