...
public class TienlenView : GameView
{
    ...
    private void chiaBai()
    {
        ...
        // 2. Set vị trí bắt đầu chia bài (từ giữa màn hình)
        Vector3 startDealPosition = new Vector3(0f, 0f, 0f);
        float baseDelay = 0.08f; // Delay giữa mỗi lá bài
        
        // 3. Loop qua từng lá bài theo thứ tự
        for (int cardIndex = 0; cardIndex < 13; cardIndex++) 
        {
            // Chia lần lượt cho từng người chơi
            for (int playerIndex = 0; playerIndex < players.Count; playerIndex++)
            {
                ...
                if (cardIndex < listCard.Count)
                {
                    Card card = listCard[cardIndex];
                    card.gameObject.SetActive(true);
                    
                    // Reset vị trí bắt đầu và góc xoay
                    card.transform.localPosition = startDealPosition;
                    card.transform.localScale = new Vector3(0.4f, 0.4f, 1);
                    card.transform.rotation = Quaternion.Euler(0, 0, -90);

                    float delay = (cardIndex * players.Count + playerIndex) * baseDelay;
                    ...
                    if (player == thisPlayer)
                    {
                        ...
                        float targetX = -((listCard.Count * 30f) / 2) + (cardIndex * 30f);
                        Vector3 targetPos = new Vector3(targetX, -250f, 0);

                        dealSequence
                            .SetDelay(delay)
                            .Append(card.transform.DOLocalMove(targetPos, 0.3f).SetEase(Ease.OutCubic))
                            .Join(card.transform.DOScale(new Vector3(0.7f, 0.7f, 1), 0.3f))
                            .Join(card.transform.DORotate(Vector3.zero, 0.3f))
                            // Thêm hiệu ứng lật bài
                            .AppendInterval(0.1f)
                            ...
                            // Thêm nhấp nhô nhẹ khi lật xong
                            .Append(card.transform.DOLocalMoveY(targetPos.y - 10f, 0.1f))
                            .Append(card.transform.DOLocalMoveY(targetPos.y, 0.1f));
                    }
                    else
                    {
                        ...
                        switch (position)
                        {
                            case 1: // Phải
                                ...
                                break;
                            case 2: // Trên
                                targetPos = new Vector3(40f + (cardIndex * 5f), 250f, 0);
                                break;
                            case 3: // Trái  
                                targetPos = new Vector3(-498f, 50f, 0);
                                break;
                        ...
                        dealSequence
                            ...
                            .Append(card.transform.DOLocalMove(targetPos, 0.3f).SetEase(Ease.OutCubic))
                            .Join(card.transform.DORotate(Vector3.zero, 0.3f))
                            .Join(DOTween.To(() => card.transform.localPosition,
                                x => card.transform.localPosition = x,
                                targetPos, 0.3f).SetEase(Ease.OutBack));
                    }

                    // Thêm sound effect cho mỗi lá bài
                    dealSequence.InsertCallback(delay, () => {
                        // TODO: Play card dealing sound
                        // playSound(Globals.SOUND_GAME.DEAL_CARD);
                    });
