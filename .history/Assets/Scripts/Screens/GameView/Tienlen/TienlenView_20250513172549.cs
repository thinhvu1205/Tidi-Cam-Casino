    ...
    public void startGame(JObject data)
    {
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            for (int j = 0; j < arr.Count; j++)
            {
                ...
                {
                    card.setTextureWithCode(0); // Úp bài
                }
        ...
        for (int cardIndex = 0; cardIndex < 13; cardIndex++) 
        {
            for (int playerIndex = 0; playerIndex < players.Count; playerIndex++)
            {
                ...
                if (cardIndex < listCard.Count)
                {
                    ...
                    if (player == thisPlayer)
                    {
                        // Người chơi chính - hiển thị mặt bài
                        float targetX = -((listCard.Count * 30f) / 2) + (cardIndex * 30f);
                        targetPos = new Vector3(targetX, -250f, 0);
                        targetScale = new Vector3(0.7f, 0.7f, 1);
                        ...
                        dealSequence
                            ...
                            .Join(card.transform.DOScale(targetScale, 0.3f))
                            .Join(card.transform.DORotate(Vector3.zero, 0.3f))
                            .AppendCallback(() => {
                                card.setTextureWithCode(card.code); // Lật bài lên
                                card.gameObject.SetActive(true); // Đảm bảo bài vẫn visible
                            });
                    ...
                    {
                        // Người chơi khác - úp bài
                        switch (position)
                        {
                            case 1: // Phải
                                targetPos = new Vector3(498f, 50f + (cardIndex * 5f), 0);
                                break;
                            case 2: // Trên
                        ...
                        targetScale = new Vector3(0.4f, 0.4f, 1);
