...
public class TienlenView : GameView
{
    ...
    public void startGame(JObject data)
    {
        ...
        mainSequence.AppendInterval(2.8f);
    ...
    {
        // 1. Setup vị trí bộ bài ở giữa
        Vector3 deckPosition = new Vector3(0f, 150f, 0f);  // Vị trí bộ bài ở giữa bàn
        float dealTime = 0.15f;  // Thời gian di chuyển mỗi lá
        float dealDelay = 0.05f; // Delay giữa các lá
        float totalDelay = 0f;

        // 2. Tạo stack bài ở giữa (visual only)
        GameObject deckVisual = new GameObject("DeckVisual");
        deckVisual.transform.SetParent(m_ContainerCards);
        deckVisual.transform.localPosition = deckPosition;
        
        // Tạo 52 lá úp để làm hiệu ứng
        for (int i = 0; i < 52; i++)
        {
            Card visualCard = getCard();
            visualCard.transform.SetParent(deckVisual.transform);
            visualCard.transform.localPosition = new Vector3(0, -i * 0.1f, 0); // Stack các lá
            visualCard.transform.localScale = new Vector3(0.4f, 0.4f, 1f);
            visualCard.transform.rotation = Quaternion.Euler(0f, 180f, 0f); // Úp bài
            visualCard.setTextureWithCode(0); // Mặt úp
            visualCard.gameObject.SetActive(true);
        }

        // 3. Chia bài theo thứ tự
        for (int cardIndex = 0; cardIndex < 13; cardIndex++) 
        {
            for (int playerIndex = 0; playerIndex < players.Count; playerIndex++)
            {
                ...
                if (cardIndex < playerCards.Count)
                {
                    ...
                    // Reset vị trí về bộ bài giữa bàn
                    ...
                    // Tính vị trí đích cho mỗi người chơi
                    Vector3 finalPos;
                    Vector3 finalScale;
                    ...
                    dealSequence
                        ...
                        .OnStart(() => {
                            // Xóa 1 lá từ bộ bài visual
                            if (deckVisual.transform.childCount > 0)
                            {
                                Destroy(deckVisual.transform.GetChild(0).gameObject);
                            }
                        })
                        .OnComplete(() => {
                            if (player == thisPlayer)
                            {
                                // Lật bài cho người chơi chính
                                card.transform.DOScaleX(0f, 0.1f).OnComplete(() => {
                                    card.setTextureWithCode(card.code);
                                    card.transform.DOScaleX(finalScale.x, 0.1f);
                                });
                            }
