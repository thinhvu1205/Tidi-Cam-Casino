...
public class TienlenView : GameView
{
    ...
    public Card spawnCard()
    {
        ...
        foreach (var cardList in ListCardPlayer)
        {
            foreach (var card in cardList)
            {
                ...
                {
                    // Add touch event khi tái sử dụng
        ...
        // Add touch event cho lá bài mới
    ...
    {
        // Chỉ add touch event cho bài của người chơi chính
        ...
        entryDown.callback.AddListener((data) => { OnCardTouch(card); });
        ...
        entryDrag.callback.AddListener((data) => {
            ...
            {
                Vector2 dragPos = ((PointerEventData)data).position;
                float dragDelta = dragPos.x - touchStartPos.x;
        ...
        entryEndDrag.callback.AddListener((data) => {
            ...
            {
                isDragging = false;
                Vector2 endPos = ((PointerEventData)data).position;
                if (Vector2.Distance(endPos, touchStartPos) <= dragThreshold)
                {
