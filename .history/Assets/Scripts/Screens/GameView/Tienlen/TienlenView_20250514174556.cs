...
public class TienlenView : GameView
{
    ...
    private void RearrangeCards()
    {
        ...
        // Sử dụng spacing, scale và vị trí giống như trong chia bài
        float spacing = 60f;
        float totalWidth = (playerCards.Count - 1) * spacing;
        float startX = -totalWidth / 2f;
        Vector3 scale = new Vector3(0.8f, 0.8f, 1f);

        // Base z-index cho layer bài - lá bên trái nhất sẽ có z-index thấp nhất
        int baseZIndex = 100;
        ...
        for (int i = 0; i < playerCards.Count; i++)
        {
            ...
            // Z-index tăng dần từ trái sang phải
            // Lá bài càng bên phải càng có z-index cao
            int zIndex = baseZIndex + (playerCards.Count - 1 - i);
            card.transform.SetSiblingIndex(zIndex);
            Debug.Log($"Card {card.code} at position {i} has z-index: {zIndex}");
        }
