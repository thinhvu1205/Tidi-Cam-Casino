...
public class TienlenView : GameView
{
    ...
    public void onClickDanhBai()
    {
        ...
        foreach (Card card in selectedCards)
        {
            ...
            // Hide card immediately for smooth transition
            card.gameObject.SetActive(false);
        }
    ...
    public void onClickSortCard() 
    {
        ...
        // Track analytics
        Analytics.LogEvent("ClickSortCard_TienLen");
        
        // Đổi kiểu sắp xếp (0,1,2)
        ...
        switch(typeSort) 
        {
            case 0: // Sắp tăng dần theo số + chất
                playerCards.Sort((x, y) => {
                    ...
                    return result != 0 ? result : x.suit.CompareTo(y.suit);
                });
                break;

            case 1: // Sắp giảm dần theo số + chất 
                playerCards.Sort((x, y) => {
                    int result = y.number.CompareTo(x.number);
                    return result != 0 ? result : x.suit.CompareTo(y.suit);
                });
                break;
            ...
            case 2: // Sắp theo chất + số
                playerCards.Sort((x, y) => {
                    int result = x.suit.CompareTo(y.suit);
                    return result != 0 ? result : x.number.CompareTo(y.number);
                });
                break;
        }
        ...
        Button btnSort = transform.Find("ButtonSort").GetComponent<Button>();
