...
public class TienlenView : GameView
{
    ...
    {
        // Vị trí bộ bài giữa bàn
        Vector3 deckPosition = new Vector3(0f, 20f, 0f);
        float dealTime = 0.3f; // Tăng thời gian để animation mượt hơn
            ...
            for (int playerIndex = 0; playerIndex < players.Count; playerIndex++)
            {
                ...
                if (cardIndex < playerCards.Count)
                {
                    ...
                    dealSequence.InsertCallback(delay, () =>
                    {
                        ...
                        card.transform.localRotation = Quaternion.Euler(0, 180f, Random.Range(-5f, 5f)); // Xoay ngẫu nhiên

                        // Tính toán vị trí đích
                        Vector3 finalPos;
                        Vector3 finalScale;
                        float flyHeight = 150f; // Độ cao khi bay
                        ...
                        path[0] = card.transform.localPosition; // Điểm bắt đầu
                        path[1] = new Vector3((finalPos.x + card.transform.localPosition.x) / 2f, 
                                            card.transform.localPosition.y + flyHeight, 
                                            0f); // Điểm giữa - cao hơn
                        path[2] = finalPos; // Điểm đích

                        // Animation sequence cho mỗi lá
                        Sequence cardSequence = DOTween.Sequence();

                        // Bay theo đường cong
                        cardSequence.Append(card.transform.DOLocalPath(path, dealTime, PathType.CatmullRom)
                            .SetEase(Ease.OutQuad));

                        // Xoay lá bài
                        if (player == thisPlayer)
                        {
                            cardSequence.Join(card.transform.DORotate(Vector3.zero, dealTime/2f)
                        ...
                        // Scale animation
                        cardSequence.Join(card.transform.DOScale(finalScale, dealTime)
                            .SetEase(Ease.OutBack));
