...
public class TienlenView : GameView
{
    ...
    private void chiaBai()
    {
        ...
        float flyHeight = Random.Range(180f, 220f); // Độ cao bay ngẫu nhiên
            ...
            for (int playerIndex = 0; playerIndex < players.Count; playerIndex++)
            {
                ...
                if (cardIndex < playerCards.Count)
                {
                    ...
                    dealSequence.InsertCallback(delay, () =>
                    {
                        ...
                        // Tính toán vị trí đích và rotation
                        Vector3 finalPos;
                        Vector3 finalRot;
                        Vector3 finalScale;
                        ...
                        {
                            // Người chơi khác - animation đẹp hơn
                            switch (position)
                            {
                                case 1: // Phải
                                    finalPos = new Vector3(498f, 80f + (cardIndex * 0.5f), 0f);
                                    finalRot = new Vector3(0f, 0f, -90f);
                                    break;
                                case 2: // Trên
                                    finalPos = new Vector3(40f + (cardIndex * 0.5f), 250f, 0f);
                                    finalRot = new Vector3(0f, 0f, -90f);
                                    break;
                                case 3: // Trái
                                    finalPos = new Vector3(-498f, 80f + (cardIndex * 0.5f), 0f);
                        ...
                        // Tạo đường bay parabol
                        Vector3[] path = new Vector3[4];
                        path[0] = card.transform.localPosition;
                        
                        // 2 điểm control cho đường cong Bezier
                        path[1] = path[0] + new Vector3(
                            (finalPos.x - path[0].x) * 0.3f,
                            flyHeight,
                            0f
                        );
                        path[2] = path[0] + new Vector3(
                            (finalPos.x - path[0].x) * 0.7f,
                            flyHeight * 0.8f,
                            0f
                        );
                        path[3] = finalPos;
                        ...
                        // Bay theo đường cong Bezier
                        cardSequence.Append(
                            card.transform.DOLocalPath(path, dealTime, PathType.CubicBezier)
                            .SetEase(Ease.OutQuad)
                        );

                        // Xoay lá bài mượt mà
                        cardSequence.Join(
                            card.transform.DORotate(finalRot, dealTime)
                            .SetEase(Ease.OutCirc)
                        );

                        // Scale với bounce effect
                        cardSequence.Join(
                            card.transform.DOScale(finalScale, dealTime)
                            .SetEase(Ease.OutBack)
                        );

                        // Sound effect với pitch ngẫu nhiên
