...
public class TienlenView : GameView
{
    ...
    {
        // 1. Setup các thông số chung
        Vector3 deckPosition = new Vector3(0f, 50f, 0f); // Nâng bộ bài lên cao hơn
        float dealTime = 0.35f;
        float dealDelay = 0.08f;
        float maxFlyHeight = 250f;
        
        Sequence dealSequence = DOTween.Sequence();

        // 2. Hiệu ứng xào bài trước khi chia
        dealSequence.Append(DOTween.Sequence()
            .Join(DOTween.To(() => deckPosition.x - 50f, x => deckPosition.x = x, deckPosition.x + 50f, 0.5f)
            .SetLoops(2, LoopType.Yoyo))
            .Join(DOTween.To(() => 0f, x => deckPosition.y = 50f + Mathf.Sin(x * Mathf.PI) * 30f, 1f, 0.5f)
            .SetLoops(2)));

        // 3. Chia bài cho từng người
            ...
            for (int playerIndex = 0; playerIndex < players.Count; playerIndex++)
            {
                ...
                if (cardIndex < playerCards.Count)
                {
                    ...
                    dealSequence.InsertCallback(delay, () => {
                        ...
                        card.transform.localRotation = Quaternion.Euler(Random.Range(-20f, 20f), 180f, Random.Range(-20f, 20f));

                        // Tính toán đường bay đẹp
                        float flyHeight = maxFlyHeight * (1f - (Mathf.Abs(playerIndex - 1.5f) / 2f));
                        Vector3 finalPos = GetFinalCardPosition(player, cardIndex, playerCards.Count);
                        Vector3 controlPoint = deckPosition + (finalPos - deckPosition) * 0.5f + Vector3.up * flyHeight;

                        // Animation sequence cho mỗi lá
                        Sequence cardSequence = DOTween.Sequence();

                        // Bay theo đường cong
                        cardSequence.Append(card.transform.DOPath(
                            new Vector3[] { deckPosition, controlPoint, finalPos },
                            dealTime,
                            PathType.CatmullRom
                        ...
                        if (player == thisPlayer)
                        {
                            cardSequence.Join(card.transform.DORotate(Vector3.zero, dealTime * 0.6f)
                                .SetEase(Ease.OutBack));
                        }
                        else 
                        {
                            cardSequence.Join(card.transform.DORotate(new Vector3(0f, 0f, -90f), dealTime)
                                .SetEase(Ease.OutCubic));
                        }

                        // Scale với bounce nhẹ
                        cardSequence.Join(card.transform.DOScale(
                            player == thisPlayer ? 0.8f : 0.45f,
                            dealTime
                        ).SetEase(Ease.OutBounce));

                        // Sound với pitch ngẫu nhiên
                        AudioManager.Instance.PlaySound("card_deal", pitch: Random.Range(0.95f, 1.05f));
                    });
                }
            }
        }
    ...
    private Vector3 GetFinalCardPosition(Player player, int cardIndex, int totalCards)
    {
        if (player == thisPlayer)
        {
            float spacing = 60f;
            ...
            return new Vector3(startX + (cardIndex * spacing), -250f, 0f);
        }

        switch (players.IndexOf(player))
        {
            case 1: // Phải
                return new Vector3(498f, 80f + cardIndex * 2f, 0f);
            case 2: // Trên  
                return new Vector3(40f + cardIndex * 2f, 250f, 0f);
            case 3: // Trái
                return new Vector3(-498f, 80f + cardIndex * 2f, 0f);
            default:
                return Vector3.zero;
