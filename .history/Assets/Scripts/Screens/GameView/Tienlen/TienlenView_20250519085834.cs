            ...
            for (int playerIndex = 0; playerIndex < players.Count; playerIndex++)
            {
                ...
                if (cardIndex < playerCards.Count)
                {
                    ...
                    dealSequence.InsertCallback(delay, () => {
                        // Setup card ban đầu từ bộ bài giữa bàn
                        card.gameObject.SetActive(true);
                        ...
                        // Animation cho người chơi chính
                        if (player == thisPlayer)
                        {
                            float spacing = 60f;
                            ...
                            cardSeq.Append(card.transform.DORotate(Vector3.zero, dealTime * 0.5f)
                                .SetEase(Ease.OutQuint));
                            cardSeq.Join(card.transform.DOLocalMove(finalPos, dealTime)
                                .SetEase(Ease.OutQuint));
                            cardSeq.Join(card.transform.DOScale(0.8f, dealTime)
                                .SetEase(Ease.OutBack));
                        }
                        // Animation cho người chơi khác
                        else 
                        {
                            ...
                            {
                                case 1: // Phải
                                    finalPos = new Vector3(498f, 80f + cardIndex * 2f, 0f);
                                    break;
                                case 2: // Trên
                                    finalPos = new Vector3(40f + cardIndex * 2f, 250f, 0f); 
                                    break;
                                case 3: // Trái
                                    finalPos = new Vector3(-498f, 80f + cardIndex * 2f, 0f);
                                    break;
                            ...
                            // Bay + xoay ngang
                            Sequence cardSeq = DOTween.Sequence();
                            cardSeq.Append(card.transform.DOLocalMove(finalPos, dealTime)
                                .SetEase(Ease.OutQuint));
                            cardSeq.Join(card.transform.DOScale(0.45f, dealTime)
                                .SetEase(Ease.OutBack));
                            cardSeq.Join(card.transform.DORotate(new Vector3(0f, 0f, -90f), dealTime)
                                .SetEase(Ease.OutQuint));
                        }

                        // Sound effect
                        playSound(Globals.SOUND_GAME.CARD_DEAL);
                    });
                }
            }
        }
