...
public class TienlenView : GameView
{
    ...
    public void finishGameTienLen(string strData)
    {
        ...
        foreach (JObject playerData in data)
        {
            ...
            {
                // Update player stats
                player.money = getLong(playerData, "M");
                player.point = getInt(playerData, "point");
                player.typeWin = getInt(playerData, "TypeWin");

                // Decode and show cards
                ...
                // Store den lang data if exists
                if (playerData["lstDenLang"] != null)
                {
                    player.lstDenLang = getJArray(playerData, "lstDenLang");
                }
            }
        }
        ...
        // Special win animation if exists
        if (playerSpecial != null)
        {
            finishSequence.AppendCallback(() => {
                m_AniCardSpecial.gameObject.SetActive(true);
                string animName = GetSpecialWinAnimationName(playerSpecial.typeWin);
    ...
    private string GetSpecialWinAnimationName(int typeWin)
    {
        switch (typeWin)
        {
            case 1: return "3doithong";
            case 2: return "4doithong";
            case 3: return "tuquy";
            case 4: return "sanh";
            case 5: return "6doi";
            case 6: return "4sam";
