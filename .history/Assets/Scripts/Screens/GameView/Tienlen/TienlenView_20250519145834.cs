...
public class TienlenView : GameView
{
    ...
    [SerializeField] public GameObject m_WinSpecial;
    ...
    public void finishGameTienLen(string strData)
    {
        ...
        foreach (JObject playerData in data)
        {
            ...
            if (player != null)
            {
                ...
                {
                    Card card = spawnCard();
                    card.setTextureWithCode((int)arrCard[i]);
                    ...
                    {
                        float spacing = 60f;
                        float totalWidth = (arrCard.Count - 1) * spacing;
                        float startX = -totalWidth / 2f;
                        card.transform.localPosition = new Vector3(startX + (i * spacing), -250f, 0f);
                        card.transform.localScale = new Vector3(0.8f, 0.8f, 1f);
                    }
                    else
                    {
                        SetCardPositionByPlayerIndex(card, players.IndexOf(player));
                ...
                if (typeWin > 0)
                {
                    ...
                    // Setup win special UI
                    m_WinSpecial.SetActive(true);
                    m_AvatarSpecial.sprite = player.playerView.avatar.image.sprite;
                    m_NameWin.text = player.displayName;

                    // Setup animation based on win type
                    switch (typeWin)
                    {
                        case 1: // 3 consecutive pairs
                            m_AniCardSpecial.node.active = true;
                            m_AniCardSpecial.AnimationState.SetAnimation(0, "3 consecutive pairs", false);
                            break;
                            
                        case 2: // 4 consecutive pairs
                            m_AniCardSpecial.node.active = true;
                        ...
                        case 4: // Straight
                            m_AniCardSpecial.node.active = true;
                            m_AniCardSpecial.AnimationState.SetAnimation(0, "straight", false);
                            break;
                        ...
                        case 6: // 4 triples
                            m_AniCardSpecial.node.active = true;
                            m_AniCardSpecial.AnimationState.SetAnimation(0, "4 triples", false);
                            break;
                    }
                }
            }
        ...
        // Final cleanup & finish
        finishSequence.AppendCallback(() => {
            // Tắt animation finish
            ...
            // Reset bài
            foreach (var cards in ListCardPlayer)
            {
