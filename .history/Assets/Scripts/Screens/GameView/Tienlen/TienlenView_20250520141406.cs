...
public class TienlenView : GameView
{
    ...
    }    private void SetDiscardCardPosition(Card card, int playerIndex, int cardIndex)
    {
        ...
        switch (playerIndex)
        {
            case 0: // Người chơi chính (bottom)
        ...
        card.transform.localPosition = new Vector3(
            // Reverse offset direction for right player (index 1)
            playerIndex == 1 ? basePos.x - offset : basePos.x + offset,
            basePos.y, 
            basePos.z
        );

        // Set proper z-index/sibling index based on play order
        int baseIndex = 1000; // Base index to ensure cards stay above other UI
        int zOffset;

        switch(playerIndex) {
            case 0: // Bottom player cards should appear on top
                zOffset = baseIndex + (cardIndex * 2);
                break;
            case 2: // Top player cards should appear below
                zOffset = baseIndex - (cardIndex * 2); 
                break;
            case 1: // Right player cards left to right
                zOffset = baseIndex - cardIndex;
                break;
            case 3: // Left player cards right to left
                zOffset = baseIndex - cardIndex;
                break;
            default:
                zOffset = baseIndex;
        ...
        card.transform.SetSiblingIndex(zOffset);
    }
    ...
    public void boLuot(string turnName, string nextTurn, bool newTurn)
    {
        ...
        //TODO: Add pass sprite and show it here
        //listSpritePass[indexPos].gameObject.SetActive(true);
        ...
        if (newTurn)
        {
            ...
            // Ẩn tất cả sprite "Pass"
            //TODO: Add pass sprites and hide them all here
    ...
    private void ShowAllCards()
    {
        ...
        {
            List<Card> cards = ListCardPlayer[players.IndexOf(player)];
