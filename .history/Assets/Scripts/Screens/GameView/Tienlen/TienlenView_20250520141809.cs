...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(Card card, int playerIndex, int cardIndex)
    {
        ...
        int baseIndex = 1000; // Base z-index to ensure cards stay above UI
        int zOffset;

        // Position and z-index based on player position
        switch (playerIndex)
        {
            case 0: // Bottom player (main)
                basePos = new Vector3(0f, -60f, 0);
                zOffset = baseIndex + (cardIndex * 2); // Highest z-index, increasing with each card
                card.transform.localPosition = new Vector3(basePos.x + offset, basePos.y, basePos.z);
                break;
            ...
            case 2: // Top player
                ...
                zOffset = baseIndex - (cardIndex * 2); // Lowest z-index, decreasing with each card
                card.transform.localPosition = new Vector3(basePos.x + offset, basePos.y, basePos.z);
                break;
        ...
        // Set z-index using sibling index
