...
public class TienlenView : GameView
{
    ...
    }    public void onClickDanhBai()
    {
        ...
        // Tạo array chứa các lá bài được chọn
        ...
        // Gửi request đánh bài lên server
        SocketSend.danhBai(arrCard);

        // Clear selected cards array
        selectedCards.Clear();

        // Disable và ẩn các nút control
        m_ButtonDiscard.SetActive(false);
        m_ButtonCancel.SetActive(false);
        m_ButtonDiscard.GetComponent<Button>().interactable = false;
        m_ButtonCancel.GetComponent<Button>().interactable = false;
    }
    ...
    {
        // Ẩn nút đánh bài và bỏ qua lúc đầu
        if (m_ButtonDiscard != null)
        {
            m_ButtonDiscard.SetActive(false);
            m_ButtonDiscard.GetComponent<Button>().interactable = false;
        }
        ...
        // Khởi tạo nút sắp xếp bài
        if (m_BtnSort != null)
        {
            m_BtnSort.SetActive(false);
        }
    }
