        ...
        foreach (JObject playerData in data)
        {
            ...
            if (player != null && player != thisPlayer)
            {
                ...
                for (int i = 0; i < playerCards.Count; i++)
                {
                    ...
                    showSequence.Insert(delay, card.transform.DOLocalMoveX((playerIndex == 1 || playerIndex == 2) ? (startX - (i * spacing)) : (startX + (i * spacing)), 0.15f));
                    card.transform.SetSiblingIndex((playerIndex == 1 || playerIndex == 2) ? 20 - i : i);

                    showSequence.Insert(delay, card.transform.DOScale(0.4f, 0.15f));
                    showSequence.Insert(delay, card.transform.DORotate(Vector3.zero, 0.15f));
