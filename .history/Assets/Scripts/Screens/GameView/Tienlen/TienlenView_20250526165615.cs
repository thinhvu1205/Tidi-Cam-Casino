...
public class TienlenView : GameView
{
    ...
    [SerializeField] private SkeletonGraphic m_AniFinish;
    [SerializeField] private SkeletonGraphic m_AniCardSpecial;
    [SerializeField] private SkeletonGraphic m_AniWinSpecial;
    [SerializeField] private Image m_AvatarSpecial;
    [SerializeField] private TextMeshProUGUI m_NameWin;
    [SerializeField] private List<GameObject> m_TxtPass;
    [SerializeField] private GameObject m_BtnSort;
    [SerializeField] private List<SkeletonGraphic> m_ListAniChay;
    [SerializeField] private GameObject m_PrefabChip;
    ...
    private List<GameObject> vtChipDenLang = new List<GameObject>();
    ...
    public void finishGameTienLen(string strData)
    {
        ...
        if (playerSpecial != null)
        {
            ...
            // Show all cards
            finishSequence.AppendCallback(() => ShowAllCards());
            finishSequence.AppendInterval(2.5f);
        }
        else
        {
        ...
        finishSequence.AppendCallback(() =>
        {
            foreach (JObject playerData in data)
            {
                ...
                var siZ = playerData["lstDenLang"]?.ToObject<JArray>()?.Count ?? 0;
                var agSub = 0L;

                if (siZ > 0)
                {
                    // Tạo chips xung quanh người thắng
                    for (int chil = 0; chil < 5 * siZ; chil++)
                    {
                        GameObject chip = Instantiate(m_PrefabChip, m_ContainerChip);
                        vtChipDenLang.Add(chip);

                        // Random vị trí quanh người chơi
                    ...
                    // Xử lý bay chip đến người thua
                    for (int j = 0; j < siZ; j++)
                    {
                        var plData = playerData["lstDenLang"][j];
                        var plLose = getPlayer((string)plData["name"]);
                        var ag = getLong((long)plData[] "ag");

                        agSub += ag;
                        plLose.playerView.effectFlyMoney(-ag);

                        // Animation 5 chip bay
                        float delayTime = 0f;
                        for (int chil = j * 5; chil < (j + 1) * 5; chil++)
                        {
                            var chip = vtChipDenLang[chil];

                            // Sequence bay chip
                            Sequence chipSequence = DOTween.Sequence();
                            chipSequence.AppendInterval(delayTime);
                            chipSequence.Append(chip.transform
                                .DOMove(plLose.playerView.transform.position, 0.3f)
                                .SetEase(Ease.OutQuint));
                    ...
                    player.playerView.effectFlyMoney(agSub);
                }
        ...
        // Cleanup chips sau khi xong
        finishSequence.AppendCallback(() =>
        {
            foreach (var chip in vtChipDenLang)
            {
    ...
    private void ShowAllCards()
    {
        ...
        foreach (var player in players)
        {
            ...
            float spacing = 30f;
            float startX = -(playerCards.Count * spacing) / 2f;
            ...
            for (int i = 0; i < playerCards.Count; i++)
            {
                ...
                showSequence.Insert(delay, card.transform.DOLocalMoveX(startX + (i * spacing), 0.3f));
