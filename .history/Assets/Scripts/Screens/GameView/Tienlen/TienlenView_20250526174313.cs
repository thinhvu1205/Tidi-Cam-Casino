...
img.SetActive(false);
    ...
    public void finishGameTienLen(string strData)
    {
        ...
        finishSequence.AppendCallback(() =>
        {
            List<GameObject> activeChips = new List<GameObject>();
            
            foreach (JObject playerData in data)
            {
                string playerName = getString(playerData, "N");
                Player player = getPlayer(playerName);
                var lstDenLang = playerData["lstDenLang"]?.ToObject<JArray>();
                int siZ = lstDenLang?.Count ?? 0;
                long agSub = 0;
                ...
                if (siZ > 0)
                {
                    ...
                    for (int chil = 0; chil < 5 * siZ; chil++)
                    {
                        ...
                        // Random vị trí quanh người thắng
