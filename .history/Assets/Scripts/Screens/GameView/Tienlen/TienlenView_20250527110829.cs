    ...
    public void finishGameTienLen(string strData)
    {
        ...
        finishSequence.AppendCallback(() => {
            foreach (JObject playerData in data) {
                ...
                // Người thua sẽ có M < 0
                if (money < 0) {
                    player.playerView.effectFlyMoney(money);
                    
                    // Tạo 10 chip bay ra từ người thua - chậm hơn
                    for (int i = 0; i < 10; i++) {
                        GameObject chip = GetChipFromPool();
                        vtChipFinish.Add(chip);
                        
                        // Set vị trí ban đầu tại người thua
                        chip.transform.position = player.playerView.transform.position;
                        
                        // Random vị trí bay ra, delay mỗi chip 
                        float randX = Random.Range(-40f, 40f);
                        float randY = Random.Range(-40f, 40f);
                        Vector3 randomPos = player.playerView.transform.position + new Vector3(randX, randY, 0);
                        
                        // Tăng duration lên 0.5s và thêm delay cho mỗi chip
                        float delay = i * 0.1f; // Delay giữa các chip
                        DOTween.Sequence()
                            .AppendInterval(delay)
                            .Append(chip.transform.DOMove(randomPos, 0.5f).SetEase(Ease.OutQuint));
                    }
                }
                // Người thắng sẽ có M > 0  
                else if (money > 0) {
                    // Tăng delay lên 3s trước khi bay về người thắng
                    DOVirtual.DelayedCall(3f, () => {
                        player.playerView.effectFlyMoney(money);
                        
                        // Animation các chip bay về người thắng - chậm hơn
                        for(int i = 0; i < vtChipFinish.Count; i++) {
                            ...
                            chipSequence.AppendInterval(delay)
                                .Append(chip.transform.DOMove(player.playerView.transform.position, 0.5f)
                                .SetEase(Ease.OutQuint))
                                .AppendCallback(() => {
