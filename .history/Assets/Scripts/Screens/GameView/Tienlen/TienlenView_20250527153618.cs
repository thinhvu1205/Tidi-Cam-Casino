...
public class TienlenView : GameView
{
    ...
    private Player winPlayer; // Thêm khai báo biến winPlayer
    ...
    private void SetDiscardCardPosition(Card card, int playerIndex, int cardIndex)
    {
        ...
        card.transform.DOLocalMove(finalPos, 0.3f)
    ...
    public void boLuot(string turnName, string nextTurn, bool newTurn)
    {
        ...
        if (indexPos >= 0 && indexPos < m_TxtPass.Count)
        {
            ...
            DOVirtual.DelayedCall(1f, () =>
            {
                ...
                {
                    m_TxtPass[indexPos].SetActive(false);
                }
            });
        }
    ...
    public void OnCardTouch(Card card)
    {
        ...
        // Nếu là lượt đầu hoặc lượt mới - không cần check gợi ý
        if (string.IsNullOrEmpty(lastTurnName))
        {
    ...
    private void SuggestCards(Card selectedCard)
    {
        ...
        // 3. Tìm các lá bài phù hợp
        List<Card> playerCards = ListCardPlayer[players.IndexOf(thisPlayer)];
        listCardSuggest.Clear();

        if (isDouble || isTriple)
        {
            // Tìm đôi/xám có số lớn hơn
            int targetNumber = lastCards[0].N;
            foreach (Card card in playerCards)
            {
                if (card.N <= targetNumber) continue;

                int count = playerCards.Count(c => c.N == card.N);
                if ((isDouble && count >= 2) || (isTriple && count >= 3))
                {
                    var sameCards = playerCards.Where(c => c.N == card.N).Take(isDouble ? 2 : 3);
                    listCardSuggest.AddRange(sameCards);
                    break;
                }
            }
        }
        ...
        {
            // Tìm sảnh có số lớn hơn
            int length = lastCards.Count;
            int targetNumber = lastCards[length - 1].N;

            for (int i = 0; i < playerCards.Count - length + 1; i++)
            {
                if (playerCards[i].N <= targetNumber) continue;

                bool valid = true;
                for (int j = 1; j < length; j++)
                {
                    if (i + j >= playerCards.Count ||
                        playerCards[i + j].N != playerCards[i].N + j ||
                        (isFlushStraight && playerCards[i + j].S != playerCards[i].S))
                    {
                        valid = false;
                        break;
                    }
                ...
                if (valid)
                {
                    listCardSuggest.AddRange(playerCards.Skip(i).Take(length));
                    break;
                }
            }
        }
        ...
        {
            // Đánh lẻ - chỉ gợi ý lá được chọn
        ...
        // 4. Highlight các lá bài được gợi ý
        HighlightSuggestedCards();
    }

    private void HighlightSuggestedCards()
    {
        foreach (Card card in listCardSuggest)
        {
            // Highlight card (e.g., raise position, change color etc)
            card.transform.DOLocalMoveY(-210f, 0.2f).SetEase(Ease.OutQuint);
        }
    }
    ...
    private void ClearSuggestion()
    {
        ...
        {
            // Remove highlight
            card.transform.DOLocalMoveY(-250f, 0.2f).SetEase(Ease.OutQuint);
        }
        listCardSuggest.Clear();
    }
    private void Update()
        ...
        finishSequence.AppendCallback(() =>
        {
            ...
            foreach (JObject playerData in data)
            {
                ...
                if (money < 0)
                {
                    ...
                    // Tạo điểm đến ngẫu nhiên cho mỗi chip
                    for (int i = 0; i < 10; i++)
                    {
                        Vector3 randomPoint = new Vector3(
                            Random.Range(-150f, 150f), // Phạm vi X rộng hơn
                            Random.Range(-80f, 80f),   // Phạm vi Y rộng hơn
                            0
                        );
                        ...
                        // Animation ném chip nhanh hơn (0.15s)  
                        float delay = i * 0.02f; // Giảm delay
                        Vector3 midPoint = (chip.transform.position + randomPoint) / 2f +
                            new Vector3(Random.Range(-30f, 30f), Random.Range(40f, 80f), 0); // Đường cong cao hơn
            ...
            if (winPlayer != null)
            {
                DOVirtual.DelayedCall(1f, () =>
                {
                    ...
                    // Animation gom tất cả chip về người thắng nhanh và đồng loạt
                    for (int i = 0; i < vtChipFinish.Count; i++)
                    {
                        var chip = vtChipFinish[i];

                        // Chia chip thành nhiều wave để tạo hiệu ứng tự nhiên
                        int wave = i / 5;
                        float delay = wave * 0.05f;

                        Vector3 start = chip.transform.position;
                        Vector3 end = winPlayer.playerView.transform.position;
                        Vector3 midPoint = (start + end) / 2f +
                            new Vector3(Random.Range(-20f, 20f), Random.Range(30f, 50f), 0);

                        Sequence chipSequence = DOTween.Sequence();
                        chipSequence.AppendInterval(delay)
                            .Append(chip.transform
                                .DOPath(new Vector3[] { start, midPoint, end }, 0.2f, PathType.CatmullRom)
                                .SetEase(Ease.OutQuint))
                            .AppendCallback(() =>
                            {
    ...
    private void ShowAllCards(JArray data)
    {
        ...
        foreach (JObject playerData in data)
        {
            ...
            for (int i = 0; i < playerCards.Count; i++)
            {
                ...
                card.transform.SetSiblingIndex((playerIndex == 1 || playerIndex == 2) ? 20 - i : i); // Đặt thứ tự hiển thị
