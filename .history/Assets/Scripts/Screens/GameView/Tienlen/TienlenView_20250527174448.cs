        ...
        foreach (JObject playerData in data)
        {
            ...
            if (playerCards.Count == 13 && discardedCards.Count == 0)
            {
                ...
                if (playerIndex >= 0 && playerIndex < m_ListAniChay.Count && m_ListAniChay[playerIndex] != null)
                {
                    ...
                    // Không tắt animation ngay mà đợi sequence kết thúc
                ...
                if (money < 0)
                {
                    ...
                    0
                        ...
                        DOTween.Sequence()
                            ...
                            .Append(chip.transform.DOMove(randomPoint, 0.3f).SetEase(Ease.OutQuint));
                    }
                }
                else if (money > 0)
                {
...
Vector3 endPoint= new Vector3(
                            positionPlayer.x + Random.Range(-30f, 30f), // Phạm vi X nhỏ hơn
                            positionPlayer.y + Random.Range(-30f, 30f), // Phạm vi Y nhỏ hơn
                            positionPlayer.z
                        );
                        Sequence chipSequence = DOTween.Sequence();
                        chipSequence.AppendInterval(delay)
                            .Append(chip.transform
                                .DOMove(winPlayer.playerView.transform.position, 0.5f)
                                .SetEase(Ease.OutQuint))
                            .AppendCallback(() =>
                            {
