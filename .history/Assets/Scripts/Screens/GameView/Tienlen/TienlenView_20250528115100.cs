...
public class TienlenView : GameView
{
    ...
    private void HighlightSuggestedCards(Card cardSelect)
    {
        // Clear previous selection first
        if (!selectedCards.Contains(cardSelect))
        {
            return;
        }
        foreach (Card card in selectedCards.ToList())
        {
            if (!listCardSuggest.Contains(card))
            {
        ...
        // Add suggested cards to selection
    ...
    {
        // Remove cards from selection and lower them
        foreach (Card card in listCardSuggest)
        {
            selectedCards.Remove(card);
            card.transform.DOLocalMoveY(-250f, 0.2f)
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                // Khi kết thúc drag, kiểm tra vị trí Y
                float currentY = selectedCard.transform.localPosition.y;
                if (currentY <= -250f && selectedCards.Contains(selectedCard))
                {
                    // Nếu lá bài ở vị trí thấp, remove khỏi danh sách chọn
    ...
    public void finishGameTienLen(string strData)
    {
        ...
        finishSequence.AppendCallback(() =>
        {
            ...
            List<Player> losePlayers = new List<Player>();
            ...
            foreach (JObject playerData in data)
            {
                ...
                if (money < 0)
                {
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        Debug.Log(" xem có chạy vào trong này ko");
                        GameObject chip = GetChipFromPool();
                        vtChipFinish.Add(chip);

                        chip.transform.position = player.playerView.transform.position;

                        // Animation bay về center với delay

                        Vector3 randomPoint = new Vector3(
                    Random.Range(-50f, 50f), // Phạm vi X rộng hơn
                    Random.Range(-50f, 50f),   // Phạm vi Y rộng hơn
                    0
                );
                        DOTween.Sequence()
                            .Append(chip.transform.DOMove(randomPoint, 1f).SetEase(Ease.OutQuint));
                    }
                }
                else if (money > 0)
                {
                ...
                DOVirtual.DelayedCall(3f, () =>
                {
                    ...
                    for (int i = 0; i < vtChipFinish.Count; i++)
                    {
                        ...
                        Vector3 endPoint = new Vector3(
                                                    positionPlayer.x + Random.Range(-20f, 20f), // Phạm vi X nhỏ hơn
                                                    positionPlayer.y + Random.Range(-20f, 20f), // Phạm vi Y nhỏ hơn
                                                    positionPlayer.z
                                                );
                        Sequence chipSequence = DOTween.Sequence();
                        chipSequence.AppendInterval(delay)
                            .Append(chip.transform
                                .DOMove(winPlayer.playerView.transform.position, 1f)
                                .SetEase(Ease.OutQuint))
                            .AppendCallback(() =>
                            {
