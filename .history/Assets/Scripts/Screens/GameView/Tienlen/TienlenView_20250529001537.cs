        ...
        finishSequence.AppendCallback(() =>
        {
            ...
            foreach (JObject playerData in data)
            {
                ...
                if (money < 0)
                {
                    ...
                    float radius = 100f;
                    Vector3 center = Vector3.zero; // bạn có thể thay bằng 1 vị trí cụ thể nếu cần
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        chip.SetActive(true); // ensure it's visible
                        ...
                        Debug.Log($"Chip from {chip.transform.position} to {destination}");

                        DOTween.Sequence()
                            .Append(chip.transform.DOMove(destination, 1.2f).SetEase(Ease.OutQuint));
                    }
