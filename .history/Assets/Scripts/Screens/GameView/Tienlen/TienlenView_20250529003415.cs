        ...
        finishSequence.AppendCallback(() =>
        {
            ...
            foreach (JObject playerData in data)
            {
                ...
                if (money < 0)
                {
                    ...
                    float radius = 30f; // bán kính khu vực trung tâm (đơn vị world position)
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        DOTween.Sequence()
                            .Append(chip.transform.DOLocalMove(destination, 0.3f).SetEase(Ease.OutQuint));
                    }
                }
                else if (money > 0)
                {
                ...
                DOVirtual.DelayedCall(3f, () =>
                {
                    ...
                    for (int i = 0; i < vtChipFinish.Count; i++)
                    {
                        ...
                        Vector3 endPoint = new Vector3(
                                                    positionPlayer.x + Random.Range(-10f, 10f), // Phạm vi X nhỏ hơn
                                                    positionPlayer.y + Random.Range(-10f, 10f), // Phạm vi Y nhỏ hơn
                                                    positionPlayer.z
                                                );
                        Sequence chipSequence = DOTween.Sequence();
                        chipSequence.AppendInterval(delay)
                            .Append(chip.transform
                                .DOMove(winPlayer.playerView.transform.position, 0.3f)
                                .SetEase(Ease.OutQuint))
                            .AppendCallback(() =>
                            {
