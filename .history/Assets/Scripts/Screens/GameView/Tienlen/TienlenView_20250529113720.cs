...
public class TienlenView : GameView
{
    ...
    private void ShowAllCards(JArray data)
    {
        ...
        foreach (JObject playerData in data)
        {
            ...
            {
                Card card = playerCards[i];
                float delay = i * 0.03f; // Delay giữa các lá bài
                showSequence.Insert(delay, card.transform.DOLocalMoveX((playerIndex == 1 || playerIndex == 2) ? startX - (i * spacing) : startX + (i * spacing), 0.05f));
