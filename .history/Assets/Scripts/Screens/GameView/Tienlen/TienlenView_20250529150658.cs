...
public class TienlenView : GameView
{
            ...
            foreach (JObject playerData in data)
            {
                ...
                if (money < 0)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        DOTween.Sequence()
                            .Append(chip.transform.DOLocalMove(destination, 0.8f).SetEase(Ease.OutQuint));
                    }
                }
                else if (money > 0)
                {
    ...
    private void ShowAllCards(JArray data)
    {
        ...
        foreach (JObject playerData in data)
        {
            ...
            // Tạo thứ tự animate: từ ngoài vào giữa
            List<int> animateOrder = new List<int>();
            int left = 0;
            int right = count - 1;
            while (left <= right)
            {
                if (left == right)
                {
                    animateOrder.Add(left);
                }
                else
                {
                    animateOrder.Add(left);
                    animateOrder.Add(right);
                }
                left++;
                right--;
            }
            ...
            {
                Card card = playerCards[realIndex];

                // Vẫn dùng index gốc để xác định vị trí
                float targetX = (playerIndex == 1 || playerIndex == 2)
                    ? startX - (realIndex * spacing)
                    : startX + (realIndex * spacing);
                card.transform.DOLocalMoveX(targetX, 0f); // Đặt trước vị trí đúng

                // Nhưng dùng animateOrder để quyết định delay
                int animationOrderIndex = animateOrder.IndexOf(realIndex);
                float delay = animationOrderIndex * 0.08f;

                showSequence.Insert(delay, card.transform.DOScale(0.4f, 0.05f));
                card.transform.SetSiblingIndex((playerIndex == 1 || playerIndex == 2) ? 20 - realIndex : realIndex);
