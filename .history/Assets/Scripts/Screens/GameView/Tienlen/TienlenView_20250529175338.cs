...
public class TienlenView : GameView
{
    ...
    public void danhBai(string turnName, string nextTurn, JArray vtCard, bool newTurn)
    {
        ...
        {
            isSpecialHand = true;
            ProcessPlayCards(player, listCard, listCardD, vtCard, position, isSpecialHand);
        }
        else
        {
        ...
        Debug.Log(check3DoiThong(listCardD) + " " +
            check4DoiThong(listCardD) + " " +
            checkTuQuy(listCardD) + " " +
            checkSetOfTwos(listCardD) + "xem có thỏa mãn cái bộ đặc biệt ko");
        if (isSpecialHand)
        {
    ...
    private void ProcessPlayCards(Player player, List<Card> listCard, List<Card> listCardD, JArray vtCard, int position, bool isSpecial = false)
    {
        if (player == thisPlayer)
        {
            for (int i = 0; i < vtCard.Count; i++)
            {
                ...
                if (cardToPlay != null)
                {
                    ...
                    SetDiscardCardPosition(cardToPlay, position, i, isSpecial);
    ...
    private void SetDiscardCardPosition(Card card, int playerIndex, int cardIndex, bool isSpecial = false)
    {
        ...
        // Thiết lập vị trí cơ bản theo vị trí người chơi
        switch (playerIndex)
        {
            case 0:
        ...
        // Nếu là bài đặc biệt → xoay + phóng to
        if (isSpecial)
        {
            // Phóng to + xoay Y một chút cho đẹp
            Sequence seq = DOTween.Sequence();
            seq.Append(card.transform.DOScale(new Vector3(0.6f, 0.6f, 1f), 0.15f)); // phóng to
            seq.Join(card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), 0.15f, RotateMode.LocalAxisAdd)); // xoay Y
            seq.Append(card.transform.DOLocalMove(finalPos, 0.25f).SetEase(Ease.OutBack));
        }
        else
        {
            // Bình thường chỉ di chuyển
    ...
    public void onClickDanhBai()
    {
        ...
        for (int i = 0; i < selectedCards.Count; i++)
        {
            ...
            Debug.Log($"[onClickDanhBai] Card: {card.code}, N = {card.N}, S = {card.S}");
            ...
            if (trigger != null)
            {
                trigger.triggers.Clear();
                Destroy(trigger);
                Debug.Log($"[onClickDanhBai] Removed EventTrigger from card {card.code}");
            }
