    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                ...
                seq.Join(card.transform.DOLocalRotate(new Vector3(0f, 360f, 0f), 0.8f, RotateMode.LocalAxisAdd));

                // Bước 2: Quay về scale và rotation ban đầu
                seq.Append(card.transform.DOScale(new Vector3(0.4f, 0.4f, 1f), 0.4f));
                seq.Join(card.transform.DOLocalRotate(Vector3.zero, 0.4f, RotateMode.Fast));
