        ...
        for (int cardIndex = 0; cardIndex < 13; cardIndex++)
        {
            for (int i = 0; i < players.Count; i++)
            {
                ...
                if (cardIndex < playerCards.Count)
                {
                    ...
                    Vector3 finalPosSelf = new Vector3(startX + (cardIndex * spacing), -250f, 0f);
                    ...
                    dealSequence.InsertCallback(delay, () =>
                    {
                        ...
                        // Bắt đầu với góc nghiêng nhẹ sang trái (Z) và bay tới mục tiêu
                        card.transform.localRotation = Quaternion.Euler(0, 0, -20f);  // nghiêng trái nhẹ

                        if (player == thisPlayer)
                        {
                            card.transform.DOLocalMove(finalPosSelf, dealTime).SetEase(Ease.OutCubic);
                            card.transform.DOScale(0.8f, dealTime);
                            card.transform.DOLocalRotate(Vector3.zero, dealTime).SetEase(Ease.OutCubic); // xoay về thẳng
                        }
                        else
                        {
