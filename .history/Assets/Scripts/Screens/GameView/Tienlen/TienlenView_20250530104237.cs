    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                ...
                // Bước 2: Vừa xoay vừa di chuyển đến vị trí cuối
                seq.Append(
                    DOTween.Sequence()
                ...
                // Bước 3: Trở lại scale và rotation ban đầu
                seq.Append(card.transform.DOScale(new Vector3(0.4f, 0.4f, 1f), 0.3f));
                seq.Join(card.transform.DOLocalRotate(Vector3.zero, 0.3f, RotateMode.Fast));
            }
            else
            {
            ...
            for (int i = 0; i < players.Count; i++)
            {
                ...
                if (cardIndex < playerCards.Count)
                {
                    ...
                    dealSequence.InsertCallback(delay, () =>
                    {
                        ...
                        // Góc nghiêng nhẹ khi bắt đầu (nghiêng trái)
                        card.transform.localRotation = Quaternion.Euler(0, 0, -15f);

                        if (player == thisPlayer)
                        {
                            // Di chuyển và hiệu ứng mượt hơn
                            card.transform.DOLocalMove(finalPos, dealTime).SetEase(Ease.OutCubic);
                            card.transform.DOScale(0.8f, dealTime).SetEase(Ease.OutBack);
                            card.transform.DOLocalRotate(Vector3.zero, dealTime).SetEase(Ease.OutCubic); // Trả về thẳng
                        }
                        else
                        {
                            Vector3 finalPosOther = Vector3.zero;
                            switch (i)
                            {
                                case 1:
                                    finalPosOther = new Vector3(498f, 80f, 0);
                                    break;
                                case 2:
                                    finalPosOther = new Vector3(40f, 250f, 0);
                                    break;
                                case 3:
                                    finalPosOther = new Vector3(-498f, 80f, 0);
                                    break;
                            ...
                            card.transform.DOLocalMove(finalPosOther, dealTime).SetEase(Ease.OutCubic);
                            card.transform.DOScale(0.45f, dealTime).SetEase(Ease.OutBack);
                            card.transform.DOLocalRotate(Vector3.zero, dealTime).SetEase(Ease.OutCubic);
                        }
                    });
                }
            }
