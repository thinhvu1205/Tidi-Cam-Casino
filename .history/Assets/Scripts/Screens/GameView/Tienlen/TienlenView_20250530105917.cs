...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            card.transform.SetSiblingIndex(cards.Count - i);
        }
    ...
    private void ShowAllCards(JArray data)
    {
        ...
        foreach (JObject playerData in data)
        {
            ...
            // Nếu cháy bài → hiển thị hiệu ứng
            ...
            if (player == thisPlayer)
            {
                RearrangeCards(); // Bài của mình tự sắp xếp lại
                continue;
            }

            // Thiết lập vị trí gốc
            float spacing = 30f;
            ...
            for (int i = 0; i < playerCards.Count; i++)
            {
                ...
                // Hiển thị và đặt texture ngay
                card.gameObject.SetActive(true);
                card.setTextureWithCode(card.code); // lật mặt bài

                // Tính vị trí đích theo người chơi
                float posX = (playerIndex == 1 || playerIndex == 2)
                    ? startX - (i * spacing)
                    : startX + (i * spacing);

                // Animation chuyển động và scale
                showSequence.Insert(delay, card.transform.DOLocalMoveX(posX, 0.3f).SetEase(Ease.OutCubic));
                showSequence.Insert(delay, card.transform.DOScale(0.4f, 0.3f).SetEase(Ease.OutCubic));

                // Đặt z-index: giảm dần từ trái sang phải
                card.transform.SetSiblingIndex(50 - i);
            }
        }
