...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        if (isSpecialHand)
        {
            ...
            specialSequence.AppendCallback(() =>
            {
                // Bổ sung hành động nếu cần sau animation
            });

            specialSequence.Play();
        }

        // Vị trí cơ bản theo người chơi
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                ...
                // Step 1: Scale lên
                seq.Append(card.transform.DOScale(Vector3.one, 0.3f));

                // Step 2: Xoay 180° đầu tiên
                seq.AppendCallback(() =>
                {
                    // Xoay nửa vòng đầu tiên
                    card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), 0.4f, RotateMode.LocalAxisAdd)
                        .SetEase(Ease.InOutSine)
                        .OnComplete(() =>
                        {
                            // Khi xoay đến mặt sau, đổi texture
                            card.setTextureWithCode(0);
                        });
                });

                // Step 3: Di chuyển + hoàn tất xoay còn lại về mặt trước
                seq.Append(
                    DOTween.Sequence()
                        .Join(card.transform.DOLocalMove(finalPos, 0.8f).SetEase(Ease.OutBack))
                        .Join(card.transform.DOLocalRotate(new Vector3(0f, 360f, 0f), 0.8f, RotateMode.LocalAxisAdd))
                );

                // Step 4: Trả lại scale nhỏ và góc chuẩn
                seq.Append(card.transform.DOScale(new Vector3(0.4f, 0.4f, 1f), 0.3f));
                seq.Join(card.transform.DOLocalRotate(Vector3.zero, 0.3f));
            }
            else
            {
            ...
            // Sắp xếp sibling theo thứ tự trái qua phải: Z giảm dần
            card.transform.SetSiblingIndex(i);
        }

        // Ẩn hiệu ứng special sau cùng để tránh bị che
        m_AniCardSpecial.gameObject.SetActive(false);
    }
