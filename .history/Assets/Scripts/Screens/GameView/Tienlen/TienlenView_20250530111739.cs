...
public class TienlenView : GameView
{
    ...
    private void ShowAllCards(JArray data)
    {
        ...
        foreach (JObject playerData in data)
        {
            ...
            // Kiểm tra cháy bài (13 lá nhưng chưa đánh lá nào)
            ...
            // Nếu là bài của chính mình → tự sắp xếp lại
            if (player == thisPlayer)
            {
                RearrangeCards();
                continue;
            ...
            // Cấu hình vị trí bắt đầu theo người chơi
            float spacing = 30f;
            ...
            for (int i = 0; i < playerCards.Count; i++)
            {
                ...
                // Tính vị trí đích theo hướng hiển thị
                float posX = reverse
                    ? startX - (i * spacing)
                    : startX + (i * spacing);
                ...
                // Hiệu ứng: di chuyển, scale, xoay mượt
                showSequence.Insert(delay, card.transform.DOLocalMove(finalPos, 0.3f).SetEase(Ease.OutBack));
                showSequence.Insert(delay, card.transform.DOScale(0.4f, 0.3f).SetEase(Ease.OutBack));
                showSequence.Insert(delay, card.transform.DOLocalRotate(Vector3.zero, 0.2f));
