...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            Vector3 finalPos = (playerIndex == 1)
                ? new Vector3(basePos.x - offset, basePos.y, basePos.z)
                : new Vector3(basePos.x + offset, basePos.y, basePos.z);
            ...
            if (isSpecialHand)
            {
                ...
                seq.Append(card.transform.DOLocalMove(Vector3.zero, 0.3f).SetEase(Ease.OutQuad));
                seq.Join(card.transform.DOScale(Vector3.one, 0.3f));

                // Step 2: Add twinkle effect
                seq.AppendCallback(() =>
                {
                    card.setEffect_Twinkle(true, 1.5f); // show sparkle for 1.5s
                });

                // Step 3: Start rotating 180°
                seq.Append(card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), 0.5f, RotateMode.LocalAxisAdd)
                    .SetEase(Ease.InOutSine)
                    .OnComplete(() =>
                    {
                        // Show back side when halfway rotated
                        card.setTextureWithCode(0);
                    }));

                // Step 4: Wait a moment (optional)
                seq.AppendInterval(0.5f);

                // Step 5: Move to final position & rotate full circle
                seq.Append(
                    DOTween.Sequence()
                        .Join(card.transform.DOLocalMove(finalPos, 0.5f).SetEase(Ease.OutBack))
                        .Join(card.transform.DOLocalRotate(new Vector3(0f, 360f, 0f), 0.5f, RotateMode.LocalAxisAdd))
                );

                // Step 6: Show real card value & return scale
                seq.AppendCallback(() =>
                {
                    card.setTextureWithCode(card.code); // show front side again
                });

                seq.Append(card.transform.DOScale(new Vector3(0.4f, 0.4f, 1f), 0.2f));
                seq.Join(card.transform.DOLocalRotate(Vector3.zero, 0.2f));
            }
            else
            {
                card.transform.DOLocalMove(finalPos, 0.2f).SetEase(Ease.InExpo);
            }

            // Z-index tăng dần từ trái qua phải
            card.transform.SetSiblingIndex(i);
        }

        // Tắt hiệu ứng đặc biệt sau cùng
        m_AniCardSpecial.gameObject.SetActive(false);
    }
        ...
        for (int cardIndex = 0; cardIndex < 13; cardIndex++)
        {
            for (int playerIndex = 0; playerIndex < players.Count; playerIndex++)
            {
                ...
                dealSequence.InsertCallback(delay, () =>
                {
                    ...
                    // Góc nghiêng ban đầu
                    capturedCard.transform.localRotation = Quaternion.Euler(0, 0, -15f);
    ...
    public void finishGameTienLen(string strData)
    {
        ...
        finishSequence.AppendCallback(() =>
        {
            ...
            foreach (var score in m_ListScore)
            {
                if (score != null)
                {
    ...
    private void ShowAllCards(JArray data)
    {
        ...
        foreach (JObject playerData in data)
        {
            ...
            // ✅ Hiển thị điểm cho tất cả người chơi
            m_ListScore[playerIndex].SetActive(true);
            m_ListScore[playerIndex].GetComponent<TextMeshProUGUI>().text = getString(playerData, "point");

            List<Card> playerCards = ListCardPlayer[playerIndex];

            // ✅ Kiểm tra cháy bài (13 lá chưa đánh)
            if (playerCards.Count == 13 && ListCardPlayerD[playerIndex].Count == 0)
            {
                Debug.Log($"Cháy bài: {playerName}");
            ...
            // ✅ Nếu là bài của mình → tự sắp xếp lại (vẫn xử lý điểm & cháy trước đó)
            if (player == thisPlayer)
            {
                RearrangeCards();
                continue;
            ...
            // ✅ Tính vị trí bắt đầu & hướng trải bài
            float spacing = 30f;
            float startX = playerIndex switch
            {
                1 => 498f,
                2 => 40f,
                3 => -498f,
            ...
            bool reverse = (playerIndex == 1 || playerIndex == 2); // hiển thị từ phải → trái

            // ✅ Sequence riêng cho từng player
            ...
            for (int i = 0; i < playerCards.Count; i++)
            {
                ...
                card.gameObject.SetActive(true);
                card.setTextureWithCode(card.code); // hiện mặt bài

                float posX = reverse ? startX - (i * spacing) : startX + (i * spacing);
                Vector3 finalPos = new Vector3(posX, card.transform.localPosition.y, 0f);

                // ✅ Đưa tween vào playerSeq theo delay
                playerSeq.AppendInterval(delay);
                playerSeq.AppendCallback(() =>
                {
                    card.transform.DOLocalMove(finalPos, 0.3f).SetEase(Ease.OutBack);
                    card.transform.DOScale(0.4f, 0.3f).SetEase(Ease.OutBack);
                    card.transform.DOLocalRotate(Vector3.zero, 0.2f);

                    int siblingIndex = reverse ? playerCards.Count - 1 - i : i;
                    card.transform.SetSiblingIndex(siblingIndex);
                });
            }

            // ✅ Gộp sequence của từng player vào chuỗi tổng
            showSequence.Join(playerSeq);
        }
