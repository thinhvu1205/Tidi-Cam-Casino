    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                ...
                // Step 2: Twinkle effect
                seq.AppendCallback(() =>
                {
                    card.setEffect_Twinkle(true, 1.5f);
                });

                // Step 3: Rotate to back
                seq.Append(card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), 0.5f, RotateMode.LocalAxisAdd)
                    .SetEase(Ease.InOutSine)
                    .OnComplete(() => { card.setTextureWithCode(0); }));

                // Step 4: Delay
                seq.AppendInterval(0.5f);

                // Step 5: Move to final pos + rotate to front
                seq.Append(
                    DOTween.Sequence()
                ...
                // Step 6: Show real card & normalize
                seq.AppendCallback(() =>
                {
            ...
            {
                // Bình thường: delay từng lá cho mượt
