...
public class TienlenView : GameView
{
    ...
    private void ShowAllCards(JArray data)
    {
        ...
        foreach (JObject playerData in data)
        {
            ...
            for (int i = 0; i < playerCards.Count; i++)
            {
                var card = playerCards[i]; // đóng băng card
                int index = i;             // đóng băng i
                float delay = index * 0.08f;

                card.gameObject.SetActive(true);
                card.setTextureWithCode(card.code);

                float posX = reverse ? startX - (index * spacing) : startX + (index * spacing);
                ...
                playerSeq.AppendCallback(() =>
                {
                    ...
                    int siblingIndex = reverse ? playerCards.Count - 1 - index : index;
                    card.transform.SetSiblingIndex(siblingIndex);
                });
            }
