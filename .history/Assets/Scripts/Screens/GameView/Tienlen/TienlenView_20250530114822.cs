    ...
    private void ShowAllCards(JArray data)
    {
        foreach (JObject playerData in data)
        {
            ...
            for (int i = 0; i < playerCards.Count; i++)
            {
                ...
                // ✅ Dùng coroutine để delay
                StartCoroutine(DelayedMoveCard(card, finalPos, siblingIndex, delay));
            }
        }
    }
    private IEnumerator DelayedMoveCard(Card card, Vector3 targetPos, int siblingIndex, float delay)
    {
        yield return new WaitForSeconds(delay);

        card.transform.DOLocalMove(targetPos, 0.3f).SetEase(Ease.OutBack);
        card.transform.SetSiblingIndex(siblingIndex);
    }
