...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        // Gán base position theo người chơi
        ...
        // Nếu là bộ đặc biệt thì hiện anim đặc biệt 2s rồi ẩn
        if (isSpecialHand)
        {
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                ...
                // Delay theo thứ tự bài
                seq.AppendInterval(delay);

                // Move to center + scale up + rotate
                seq.Append(
                    DOTween.Sequence()
                        .Join(card.transform.DOLocalMove(Vector3.zero, 0.4f).SetEase(Ease.OutQuad))
                        .Join(card.transform.DOScale(Vector3.one, 0.4f))
                        .Join(card.transform.DOLocalRotate(new Vector3(0f, 360f, 0f), 0.4f, RotateMode.LocalAxisAdd))
                );
                ...
                // Move to final position + rotate + scale down
                seq.Append(
                    DOTween.Sequence()
                ...
                // Reset texture sau khi xoay
                seq.AppendCallback(() =>
                {
                    card.setTextureWithCode(code);
                });

                // Đảm bảo normalize rotation
