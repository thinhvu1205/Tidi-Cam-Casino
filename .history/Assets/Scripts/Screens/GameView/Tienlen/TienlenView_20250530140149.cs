...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        // Tính vị trí trung tâm không bị chồng lá
        float centerSpacing = 50f;
        float totalWidth = (cards.Count - 1) * centerSpacing;
        Vector3 centerOrigin = Vector3.zero - new Vector3(totalWidth / 2f, 0f, 0f); // Giữa bàn, các lá lệch đều
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                ...
                // Move to center + scale up + rotate 180 (hiện mặt lưng)
                seq.Append(
                    DOTween.Sequence()
                        .Join(card.transform.DOLocalMove(centerPos, 0.3f).SetEase(Ease.OutQuad))
                        .Join(card.transform.DOScale(Vector3.one, 0.3f))
                        .Join(card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), 0.3f, RotateMode.LocalAxisAdd))
                );

                // Set mặt lưng sau 180 độ
                seq.AppendCallback(() =>
                {
                    card.setTextureWithCode(0); // Mặt lưng
                });

                // Tiếp tục xoay nốt 180 độ còn lại (thành 360 tổng)
                seq.Append(card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), 0.3f, RotateMode.LocalAxisAdd));

                // Twinkle effect
                seq.AppendCallback(() =>
                {
                    card.setEffect_Twinkle(true, 1.2f);
                });

                seq.AppendInterval(0.2f);

                // Move về vị trí cuối cùng + xoay thêm + scale nhỏ lại
                seq.Append(
                    DOTween.Sequence()
                        .Join(card.transform.DOLocalMove(finalPos, 0.5f).SetEase(Ease.OutBack))
                        .Join(card.transform.DOScale(new Vector3(0.4f, 0.4f, 1f), 0.5f))
                        .Join(card.transform.DOLocalRotate(new Vector3(0f, 360f, 0f), 0.5f, RotateMode.LocalAxisAdd))
                );

                // Đổi lại texture thật
                seq.AppendCallback(() =>
                {
                    card.setTextureWithCode(code);
                });

                // Reset góc xoay về 0 cho chắc
