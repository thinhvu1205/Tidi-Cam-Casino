...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        // ✅ Hiện anim đặc biệt nếu là hand đặc biệt
        if (isSpecialHand)
        {
        ...
        // ✅ Vị trí giữa bàn để không bị chồng
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                ...
                float moveToCenterTime = 0.8f;
                float moveToFinalTime = 0.4f;

                seq.AppendInterval(delay);

                // 👉 Move to center + scale to 1 + rotate 180 (half)
                seq.Append(
                    DOTween.Sequence()
                        .Join(card.transform.DOLocalMove(centerPos, moveToCenterTime).SetEase(Ease.OutQuad))
                        .Join(card.transform.DOScale(Vector3.one, moveToCenterTime))
                        .Join(card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), moveToCenterTime * 0.5f, RotateMode.LocalAxisAdd))
                );

                // 👉 After 180° → set mặt lưng
                seq.AppendCallback(() =>
                {
                    card.setTextureWithCode(0);
                });

                // 👉 Rotate tiếp 180° (tổng 360°)
                seq.Append(card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), moveToCenterTime * 0.5f, RotateMode.LocalAxisAdd));

                // 👉 Sau khi đến giữa bàn → set lại mặt thật + Twinkle
                seq.AppendCallback(() =>
                {
                    card.setTextureWithCode(code);
                      seq.Append(card.transform.DOLocalRotate(new Vector3(0,180,0), 0.2f));
                    card.setEffect_Twinkle(true, 1.2f);
                });

                seq.AppendInterval(0.3f);

                // 👉 Move về vị trí đích + scale nhỏ lại (không xoay)
                seq.Append(
                    DOTween.Sequence()
