  ...
  private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
{
    ...
    // ✅ Tính vị trí giữa bàn (không chồng)
    ...
    for (int i = 0; i < cards.Count; i++)
    {
        ...
        if (isSpecialHand)
        {
            ...
            // 👉 Move + Scale + Rotate đến giữa bàn (xoay nửa vòng đầu)
            seq.Append(
                DOTween.Sequence()
                    ...
                    .Join(card.transform.DOLocalRotate(new Vector3(0f, 90f, 0f), moveToCenterTime * 0.5f, RotateMode.LocalAxisAdd))
            );

            // 👉 Đổi sang mặt lưng ở 90°
            seq.AppendCallback(() =>
            {
                card.setTextureWithCode(0); // Mặt lưng
            });

            // 👉 Xoay tiếp 90° → thành 180°
            seq.Append(card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), moveToCenterTime * 0.5f, RotateMode.LocalAxisAdd));

            // 👉 Set lại mặt thật + Twinkle
            seq.AppendCallback(() =>
            {
            ...
            // 👉 Di chuyển về vị trí đích + thu nhỏ
            seq.Append(
                DOTween.Sequence()
