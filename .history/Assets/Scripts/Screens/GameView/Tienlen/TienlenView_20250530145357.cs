               ...
               if (isSpecialHand)
{
    ...
    DOVirtual.DelayedCall(delay, () =>
    {
        ...
        float rotateStepTime = 0.3f;
        float moveToFinalTime = 0.4f;

        Sequence seq = DOTween.Sequence();

        // 👉 Di chuyển đến giữa bàn + scale to 1
        ...
        // 👉 Xoay đến 90° (nửa đầu) → đổi sang mặt lưng
        seq.Append(card.transform.DOLocalRotate(new Vector3(0f, 90f, 0f), rotateStepTime, RotateMode.LocalAxisAdd));
        seq.AppendCallback(() => card.setTextureWithCode(0));

        // 👉 Xoay tiếp đến 270° (thêm 180°) → đổi sang mặt thật + Twinkle
        seq.Append(card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), rotateStepTime, RotateMode.LocalAxisAdd));
        seq.AppendCallback(() =>
        {
        ...
        // 👉 Đợi chút rồi move về vị trí đích + scale nhỏ lại
