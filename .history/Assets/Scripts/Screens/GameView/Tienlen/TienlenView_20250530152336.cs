...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        // ✅ Vị trí giữa bàn không chồng lá
        float centerSpacing = 50f;
        float totalWidth = (cards.Count - 1) * centerSpacing;
        Vector3 centerOrigin = Vector3.zero - new Vector3(totalWidth / 2f, 0f, 0f);
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                ...
                {
                    float moveToCenterTime = 0.8f;   // thời gian move + scale to lớn
                    float totalRotateTime = 1.0f;    // thời gian xoay 360°
                    float moveToFinalTime = 0.4f;    // thời gian move về vị trí cuối

                    bool flippedToBack = false;
                    bool flippedToFront = false;

                    Sequence seq = DOTween.Sequence();

                    // Bước 1: Move ra giữa bàn + scale to lớn
                    seq.Append(card.transform.DOLocalMove(centerPos, moveToCenterTime).SetEase(Ease.OutQuad));
                    seq.Join(card.transform.DOScale(Vector3.one, moveToCenterTime));

                    // Bật twinkle ngay lúc bắt đầu move + scale (ở thời điểm delay)
                    seq.Insert(0, DOVirtual.DelayedCall(0f, () => card.setEffect_Twinkle(true, 1.2f)));

                    // Bước 2: Xoay 360° liên tục, đồng thời đổi mặt bài đúng lúc
                    seq.Append(card.transform.DOLocalRotate(new Vector3(0f, 360f, 0f), totalRotateTime, RotateMode.LocalAxisAdd)
                        .SetEase(Ease.InOutSine)
                        .OnUpdate(() =>
                        {
                            float y = card.transform.localEulerAngles.y % 360f;

                            if (!flippedToBack && y > 85f && y < 95f)
                            {
                                card.setTextureWithCode(0); // mặt lưng
                                flippedToBack = true;
                            }
                            else if (!flippedToFront && y > 265f && y < 275f)
                            {
                                card.setTextureWithCode(code); // mặt thật
                                flippedToFront = true;
                            }
                        }));

                    // Bước 3: Tắt twinkle ngay sau khi xoay xong
                    seq.AppendCallback(() => card.setEffect_Twinkle(false,0f));

                    // Bước 4: Đợi chút rồi move về vị trí cuối + scale nhỏ lại
                    ...
                    // Bước 5: Reset lại góc xoay
                    seq.Append(card.transform.DOLocalRotate(Vector3.zero, 0.2f));
                });
    ...
    private void ShowAllCards(JArray data)
    {
        foreach (JObject playerData in data)
        {
            ...
            bool reverse = (playerIndex == 1 || playerIndex == 2);
            ...
            for (int i = 0; i < playerCards.Count; i++)
            {
                ...
                card.transform.DOLocalMove(finalPos, 0.1f).SetEase(Ease.OutBack);
                card.transform.SetSiblingIndex(siblingIndex);
