...
public class TienlenView : GameView
{
                ...
                DOVirtual.DelayedCall(delay, () =>
   {
       ...
       ? cards.Count - 1 - i
       : i;
       card.transform.SetSiblingIndex(siblingIndex); // 👉 Đặt TRƯỚC animation
       ...
       float halfRotateTime = animTime / 2f;

       Sequence seq = DOTween.Sequence();

       // Twinkle effect bật sớm
       seq.Insert(0f, DOVirtual.DelayedCall(0f, () => card.setEffect_Twinkle(true, 1.6f)));

       // Move đến center + scale to
       ...
       // 🔁 Lật bài kiểu "2 nửa": xoay 0 -> 90, đổi mặt, rồi 90 -> 0
       seq.Join(card.transform.DOLocalRotate(new Vector3(0f, 90f, 0f), halfRotateTime)
           .SetEase(Ease.InOutSine)
           .OnComplete(() =>
           {
               card.setTextureWithCode(code); // lật mặt thật tại 90°
           }));

       seq.Append(card.transform.DOLocalRotate(Vector3.zero, halfRotateTime)
           .SetEase(Ease.InOutSine));

       // Đợi chút, rồi move về vị trí cuối + scale nhỏ lại
       ...
       // Reset rotation (đề phòng lệch xoay nhỏ)
