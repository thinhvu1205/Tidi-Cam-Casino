...
public class TienlenView : GameView
{
                ...
                DOVirtual.DelayedCall(delay, () =>
 {
     int siblingIndex = (playerIndex == 1)
         ? cards.Count - 1 - i
         : i;
     card.transform.SetSiblingIndex(siblingIndex); // Set thứ tự hiển thị ngay từ đầu

     int code = card.code;

     // Set mặt sau ngay từ đầu
     ...
     // Xoay 180° liền mạch để lật bài, đổi mặt ở giữa (90°)
     bool flippedToFront = false;
     seq.Join(
         card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), animTime, RotateMode.LocalAxisAdd)
             .SetEase(Ease.InOutSine)
             .OnUpdate(() =>
             {
                 float y = card.transform.localEulerAngles.y % 360f;
                 if (!flippedToFront && y > 85f && y < 95f)
                 {
                     card.setTextureWithCode(code); // Lật sang mặt thật tại ~90°
                     flippedToFront = true;
                 }
             })
     );

     // Đợi chút, rồi di chuyển về vị trí cuối + scale nhỏ lại
     ...
     // Reset lại rotation về 0 sau cùng
