                ...
                DOVirtual.DelayedCall(delay, () =>
{
    ...
    // 🔧 1. Set mặt sau ngay từ đầu và xoay card về y = 180°
    // Đặt bài ở trạng thái úp đúng ngay từ đầu
    card.setTextureWithCode(0);
    card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), 0f).Complete();  // Không dùng localRotation trực tiếp
    ...
    seq.Join(
        card.transform.DOLocalRotate(new Vector3(0f, 180f, 0f), animTime, RotateMode.LocalAxisAdd)
            ...
            .OnUpdate(() =>
            {
                ...
                if (!flippedToFront && y > 265f && y < 275f) // ~90° so với mặt sau
                {
                    card.setTextureWithCode(code); // đổi sang mặt thật
                    flippedToFront = true;
                }
            })
    );

    // ⏳ Đợi chút rồi move về vị trí cuối
