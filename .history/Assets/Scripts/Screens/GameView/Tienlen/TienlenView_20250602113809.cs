        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                ...
                DOVirtual.DelayedCall(delay, () =>
                {
                    ...
                    // 🔁 Flip 2 bước
                    seq.Join(card.transform.DOLocalRotate(new Vector3(0f, 90f, 0f), animTime / 2f, RotateMode.LocalAxisAdd)
                        .SetEase(Ease.InOutSine)
                        .OnComplete(() =>
                        {
                            card.setTextureWithCode(code); // Đổi mặt tại 270°
                        })
                    );
