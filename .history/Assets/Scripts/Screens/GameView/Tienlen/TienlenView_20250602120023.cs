...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSpecialHand)
            {
                // 🔧 Set trạng thái mặt úp ngay tại đây – TRƯỚC cả DOVirtual.DelayedCall
                card.setTextureWithCode(0); // đặt mặt úp
                card.transform.localRotation = Quaternion.Euler(0f, 180f, 0f); // bài úp quay 180°
                ...
                DOVirtual.DelayedCall(delay, () =>
 {
     ...
     // 🧊 Đặt trạng thái mặt úp và xoay 180°
     ...
     // 🔁 Flip trong khi di chuyển ra giữa bàn
     // Xoay từ 180 -> 90 và đổi mặt
     seq.Join(card.transform.DOLocalRotate(new Vector3(0f, 90f, 0f), animTime / 2f).SetEase(Ease.InOutSine));
     seq.AppendCallback(() =>
     {
         card.setTextureWithCode(code); // Đổi mặt tại 90°
     });
     // Xoay tiếp từ 90 -> 0
     seq.Append(card.transform.DOLocalRotate(new Vector3(0f, 0f, 0f), animTime / 2f).SetEase(Ease.InOutSine));
