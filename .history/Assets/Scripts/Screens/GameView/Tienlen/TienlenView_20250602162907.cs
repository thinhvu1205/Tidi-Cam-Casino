...
public class TienlenView : GameView
{
    ...
    private List<string> ListPlayerLeave = new List<string>(); // Danh sách người chơi bên trái
                ...
                DOVirtual.DelayedCall(delay, () =>
 {
     ...
     seq.Join(card.transform.DOLocalRotate(new Vector3(0f, 89f, 0f), animTime / 2f).SetEase(Ease.InOutSine));
     seq.AppendCallback(() =>
     {
         card.setTextureWithCode(cardCodeSpecial[i]); // Đổi mặt tại 90°
     });
     // Xoay tiếp từ 90 -> 0
    ...
    public void HandleFinishGame()
    {
        ...
        foreach (var player in ListPlayerLeave)
        {
            removePlayer(player);
        }
