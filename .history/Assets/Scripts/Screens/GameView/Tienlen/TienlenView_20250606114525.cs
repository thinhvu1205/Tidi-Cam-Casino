...
public class TienlenView : GameView
{
    ...
    private void setNumberCardLast(bool isFinish = false)
    {
        for (int i = 0; i < players.Count; i++)
        {
            ...
            Debug.Log("check ở đây " + ListCardPlayer[i].Count + "" + isFinish + "" + (agTable > 1000));
            if (ListCardPlayer[i - 1].Count == 0 || isFinish || agTable > 1000)
            {
                m_NumberCardLast[i - 1].text = "";
            }
            else
            {
                Debug.Log("ở đây có tí");
    ...
    public void danhBaiError(string error)
    {
        ...
        {
            Debug.Log("check ở chỗ thisplayer " + isCenterButton);
            handleButton(true, true, isCenterButton);
        }
    }
                ...
                DOVirtual.DelayedCall(delay, () =>
 {
    ...
    public void handleButton(bool isCancel, bool isDiscard, bool isCenter)
    {
        ...
        Debug.Log(isCenterButton + " xem ở chỗ này");
