...
public class TienlenView : GameView
{
    ...
    {
        // Đảm bảo thisPlayer luôn ở vị trí đầu tiên
        players.Remove(thisPlayer);
        players.Insert(0, thisPlayer);

        // Sắp xếp các người chơi còn lại theo thứ tự ngược chiều kim đồng hồ
        for (int i = 0; i < players.Count; i++)
        {
            if (players[i] == null) continue;
            
            // Tính toán vị trí hiển thị tương đối
            int displayIndex = i;
            
            // Điều chỉnh vị trí cho game Tiến Lên khi có 2 người chơi
            ...
            if (displayIndex < listPosView.Count)
            {
                players[i].transform.position = listPosView[displayIndex].position;
                players[i].indexDynamic = displayIndex;
            }
        }
    }
