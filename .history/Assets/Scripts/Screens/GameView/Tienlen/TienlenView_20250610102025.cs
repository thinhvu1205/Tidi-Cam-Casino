...
public class TienlenView : GameView
{
    ...
    private void AddCardTouchEvents(Card card)
    {
        ...
        {
            if (isPlay && turnNameCurrent == thisPlayer.namePl)
            {
                OnCardTouch(card);
            }
        });
        ...
        {
            if (isPlay && turnNameCurrent == thisPlayer.namePl && isDragging && selectedCard == card)
            {
                // Handle drag
            }
        });
                ...
                DOVirtual.DelayedCall(delay, () =>
 {
    ...
    public override void handleLTable(JObject data)
    {
        ...
        Debug.Log(isPlay + "xem là đến đây thì nó chạy j" + player.namePl);
        if (isPlay && player != thisPlayer)
        {
