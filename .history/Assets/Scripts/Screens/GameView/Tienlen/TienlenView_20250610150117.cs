    ...
    private void SuggestCards(Card selectedCard)
    {
        ...
        if (isDouble || isTriple)
        {
            ...
            if (isTriple)
            {
                ...
                {
                    if (g.Key == 2) return 100;
                    if (g.Key == 14) return 99;
                    return g.Key;
                });
            }
            else
        ...
        else if (isStraight || isFlushStraight)
        {
            ...
            for (int len = length; len <= sortedCards.Count; len++)
            {
                for (int i = 0; i <= sortedCards.Count - len; i++)
                {
                    List<Card> seq = sortedCards.Skip(i).Take(len).ToList();

                    if (seq.Any(c => c.N == 2)) continue;
                    ...
                    if (seq[^1].N > targetNumber && seq.Contains(selectedCard))
                    {
