    ...
    private void SuggestCards(Card selectedCard)
    {
        ...
        if (isDouble || isTriple)
        {
            ...
            var grouped = playerCards
                ...
                .Where(g => g.Any(c => c == selectedCard));

            if (isTriple)
            {
                grouped = grouped.OrderByDescending(g =>
                {
                    if (g.Key == 2) return 100;     // Ưu tiên heo
                    if (g.Key == 14) return 99;     // Ưu tiên Át
                    return g.Key;
                });
            }
            else
        ...
        // ✅ Gợi ý sảnh / thùng phá sảnh
        else if (isStraight || isFlushStraight)
        {
            int length = lastCards.Count;
            int targetNumber = lastCards[^1].N;

            List<Card> usableCards = new(playerCards);
            usableCards = usableCards.Where(c => c.N != 2).ToList(); // bỏ 2 nếu không cho vào sảnh

            List<List<Card>> candidateSeqs = new();

            // Tạo tất cả tổ hợp có thể từ usableCards
            for (int i = 0; i < usableCards.Count; i++)
            {
                List<Card> seq = new();
                HashSet<int> usedN = new(); // Không trùng giá trị N
                seq.Add(usableCards[i]);
                usedN.Add(usableCards[i].N);

                int currentN = usableCards[i].N;

                for (int j = 0; j < usableCards.Count && seq.Count < length; j++)
                {
                    if (i == j) continue;
                    Card c = usableCards[j];
                    if (usedN.Contains(c.N)) continue;

                    if (c.N == currentN + 1 &&
                        (!isFlushStraight || c.S == seq[0].S))
                    {
                        seq.Add(c);
                        usedN.Add(c.N);
                        currentN = c.N;
                ...
                if (seq.Count == length &&
                    ...
                    seq.Contains(selectedCard))
                {
                    candidateSeqs.Add(seq);
                }
            }

            var bestSeq = candidateSeqs
                .OrderBy(seq => seq[0].N)
                .ThenBy(seq => seq[^1].N)
                .FirstOrDefault();
            ...
            if (bestSeq != null)
            {
                listCardSuggest.AddRange(bestSeq);
            }
        }
        // ✅ Gợi ý bài lẻ (một lá)
        else
