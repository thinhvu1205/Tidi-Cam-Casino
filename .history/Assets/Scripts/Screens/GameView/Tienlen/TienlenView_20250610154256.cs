                ...
                DOVirtual.DelayedCall(delay, () =>
 {
    ...
    private void SuggestCards(Card selectedCard)
    {
        ...
        if (isDouble || isTriple)
        {
            ...
            int requiredCount = isDouble ? 2 : 3;
            ...
            {
                // Cho phép chọn bất kỳ lá nào trong bộ (không so sánh object mà so N)
                if (group.Any(c => c.N == selectedCard.N))
                {
                    listCardSuggest.AddRange(group.Take(requiredCount));
                    break;
                }
            }
        }
        // ✅ Gợi ý sảnh hoặc thùng phá sảnh
        else if (isStraight || isFlushStraight)
        {
            int length = lastCards.Count;
            int targetNumber = lastCards[^1].N;

            // Lọc bỏ 2 nếu không được vào sảnh
            var usableCards = playerCards
                .Where(c => c.N != 2)
                .ToList();
            ...
            // Lọc các dãy hợp lệ chứa selectedCard (so N & S), và > bộ trước
            var valid = sequences
                .Where(seq => seq.Any(c => c.N == selectedCard.N && c.S == selectedCard.S))
            ...
            if (valid.Count > 0)
                listCardSuggest.AddRange(valid[0]);
        }
        // ✅ Gợi ý bài lẻ
        else
        {
    ...
    private List<List<Card>> GetAllValidStraightCombos(List<Card> cards, int length, bool sameSuit)
    {
        // Lấy 1 lá duy nhất cho mỗi giá trị N để tránh trùng N trong sảnh
        var grouped = cards
            .GroupBy(c => c.N)
            .Select(g => g.First()) // lấy 1 lá bất kỳ trong số những lá cùng N
            .OrderBy(c => c.N)
            .ToList();

        List<List<Card>> result = new();

        for (int i = 0; i <= grouped.Count - length; i++)
        {
            var seq = grouped.Skip(i).Take(length).ToList();
            bool isValid = true;
            ...
            for (int j = 1; j < seq.Count; j++)
            {
                ...
                // Nếu là thùng phá sảnh, kiểm tra đồng chất
                if (sameSuit && seq[j].S != seq[0].S)
                {
                    isValid = false;
                    break;
                }
