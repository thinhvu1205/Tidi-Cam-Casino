                ...
                DOVirtual.DelayedCall(delay, () =>
 {
    ...
    private void SuggestCards(Card selectedCard)
    {
        ...
        if (isDouble || isTriple)
        {
            ...
            // Lấy các nhóm theo số lượng, có >= neededCount lá, giá > targetNumber
            var groups = playerCards
                .GroupBy(c => c.N)
                .Where(g => g.Key > targetNumber && g.Count() >= neededCount)
                .OrderBy(g => g.Key);

            foreach (var group in groups)
            {
                // Lấy chính xác các lá trong nhóm
                var groupCards = group.ToList();

                // Kiểm tra xem nhóm có chứa đúng selectedCard không (so sánh object)
                if (groupCards.Contains(selectedCard))
                {
                    listCardSuggest.AddRange(groupCards.Take(neededCount));
                    break;
                }
            }
        }
        else if (isStraight || isFlushStraight)
        {
            ...
            // Loại bỏ lá 2 vì không được nằm trong sảnh
            List<Card> usableCards = playerCards.Where(c => c.N != 2).ToList();

            // Tìm tất cả sảnh hợp lệ
            var sequences = GetAllValidStraightCombos(usableCards, length, isFlushStraight);

            // Lọc ra các sảnh có chứa đúng lá selectedCard (theo object) và lớn hơn targetNumber
            var validSequences = sequences
                .Where(seq => seq.Contains(selectedCard))
                .Where(seq => seq[^1].N > targetNumber)
                .OrderBy(seq => seq[0].N) // ưu tiên bộ nhỏ nhất
                .ThenBy(seq => seq.Count)
                .ToList();
