...
public class TienlenView : GameView
{
    ...
    private void SetDiscardCardPosition(List<Card> cards, int playerIndex)
    {
        ...
        if (playerIndex == 1)
        {
            cards.Sort((a, b) => b.N.CompareTo(a.N));
        }
        else
        {
                ...
                DOVirtual.DelayedCall(delay, () =>
 {
    ...
    private void SuggestCards(Card selectedCard)
    {
        ...
        bool isFlushStraight = checkThungPhaSanhTL(lastCards); // không phân biệt chất ở đây
        ...
        if (isDouble || isTriple)
        {
            ...
            {
                listCardSuggest.AddRange(validGroups[0].Take(neededCount));
            }
        }
        else if (isStraight || isFlushStraight)
        {
            ...
            // Bỏ bài 2 ra khỏi sảnh
            List<Card> usableCards = playerCards.Where(c => c.N != 2).ToList();

            // Tìm các sảnh hợp lệ
            var sequences = GetAllValidStraightCombos(usableCards, length);

            var validSequences = sequences
                .Where(seq => seq.Contains(selectedCard))
                .Where(seq => seq[^1].N > targetNumber)
                .OrderBy(seq => seq[^1].N) // theo quân cuối (mạnh hơn sảnh trước)
                .ThenBy(seq => seq[0].N)
                .ToList();
        ...
        {
            if (GetCardStrength(selectedCard.N) > GetCardStrength(lastCards[0].N))
            {
    ...
    private List<List<Card>> GetAllValidStraightCombos(List<Card> cards, int length)
    {
        List<List<Card>> result = new List<List<Card>>();

        // Loại bỏ quân bài bị trùng số để lấy 1 lá mỗi số
        var cardsByNumber = cards
            .GroupBy(c => c.N)
            .Where(g => g.Key != 2) // 2 không hợp lệ trong sảnh
            .Select(g => g.First()) // chỉ lấy 1 lá đại diện cho mỗi số
            .OrderBy(c => c.N)
            .ToList();

        for (int i = 0; i <= cardsByNumber.Count - length; i++)
        {
            List<Card> candidate = new List<Card>();
            candidate.Add(cardsByNumber[i]);

            int prev = cardsByNumber[i].N;

            for (int j = i + 1; j < cardsByNumber.Count && candidate.Count < length; j++)
            {
                if (cardsByNumber[j].N == prev + 1)
                {
                    candidate.Add(cardsByNumber[j]);
                    prev = cardsByNumber[j].N;
                }
                else if (cardsByNumber[j].N > prev + 1)
                {
    ...
    /// Hàm tìm tất cả sảnh hợp lệ từ bộ bài (có thể cùng hoặc khác chất nếu isFlush = false)
    private List<List<Card>> GetAllValidStraightCombos(List<Card> cards, int length, bool isFlush)
    {
        List<List<Card>> result = new List<List<Card>>();

        // Sắp xếp theo số và chất
        var sortedCards = cards.OrderBy(c => c.N).ThenBy(c => c.S).ToList();

        for (int i = 0; i <= sortedCards.Count - length; i++)
        {
            for (int len = length; len <= sortedCards.Count - i; len++)
            {
                var combo = sortedCards.Skip(i).Take(len).ToList();

                if (combo.Count < length) break;

                // Kiểm tra sảnh hợp lệ
                bool valid = true;
                for (int j = 1; j < combo.Count; j++)
                {
                    if (combo[j].N != combo[j - 1].N + 1)
                    {
                        valid = false;
                        break;
                    }
                    if (isFlush && combo[j].S != combo[0].S)
                    {
                        valid = false;
                        break;
                    }
                ...
                // Tránh duplicate (tổ hợp trùng lặp)
                bool duplicate = result.Any(existing =>
                    existing.Count == combo.Count && !existing.Except(combo).Any() && !combo.Except(existing).Any());
